...
func (ch Challenge) Run() (string, error) {
	...
	var err error
	if ch.InputFileName != "" {
		input, err = readInputFile(ch.Year, ch.Day, ch.InputFileName)
		if err != nil {
			return "", err
		}
		ch.Input = input
...
func readInputFile(year int, day int, inputFileName string) ([]string, error) {
	pwd, _ := os.Getwd()
	fp := filepath.Join(pwd, "challenges", fmt.Sprintf("%d", year), fmt.Sprintf("day%.2d", day), inputFileName)
	file, err := os.Open(fp)
	if err != nil {
		return nil, err
	}
