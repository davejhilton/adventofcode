...
func day03_part1(input []string) (string, error) {
	binLists := day03_parse(input)
...
func day03_part2(input []string) (string, error) {
	binLists := day03_parse(input)
	...
	log.Debugf("Possibilities are:\n%s\n\n\n", day03_doubleSliceToString(o2list))
	o2soFar := ""
	for i := 0; i < bitCount; i += 1 {
		...
		log.Debugf("Bit [%d]: %d/%d have 1 bit, so most common is '%d'.\n%s\nPossibilities are:\n%s\n\n\n", i, sum, n, bit, o2soFar, day03_doubleSliceToString(o2list))

		if len(o2list) == 1 {
			o2gen = day03_binSliceToDec(o2list[0])
	...
	for i := 0; i < bitCount; i += 1 {
		...
		log.Debugf("Bit [%d]: %d/%d have 1 bit, so least common is '%d'.\n%s\nPossibilities are:\n%s\n\n\n", i, sum, n, bit, co2soFar, day03_doubleSliceToString(co2list))

		if len(co2list) == 1 {
			co2scrub = day03_binSliceToDec(co2list[0])
...
func day03_binSliceToDec(bin []int) int {
	r := 0
	bitCount := len(bin)
	for i, d := range bin {
		if d == 1 {
...
func day03_doubleSliceToString(bl [][]int) string {
	str := ""
	for i, l := range bl {
		if i > 0 {
			str = fmt.Sprintf("%s\n", str)
...
func day03_parse(input []string) [][]int {
...
func init() {
	challenges.RegisterChallengeFunc(2021, 3, 1, "day03.txt", day03_part1)
	challenges.RegisterChallengeFunc(2021, 3, 2, "day03.txt", day03_part2)
}
