...
func day18_part2(input []string) (string, error) {
	expressions := day18_parse(input)
	log.Debugln("*****************")
	sum := 0
	for _, e := range expressions {
...
func day18_parse(input []string) []day18_token {
	expressions := make([]day18_token, 0, len(input))
	for _, line := range input {
		exprStack := make([]day18_token, 0)
		exprStack = append(exprStack, day18_token{
			Type:      ROOT_EXPRESSION,
			SubTokens: make([]day18_token, 0),
		})
		curIdx := 0
...
type day18_token struct {
	Type      string
	IntVal    int
	SubTokens []day18_token
}

func (t day18_token) String() string {
	var str string
	switch t.Type {
...
func (t day18_token) HandleAltPrecedence() day18_token {
	log.Debugf("SIMPLIFYING: %s\n", t)
	var newT day18_token
	switch t.Type {
	case NUMBER:
		newT = t
	case ADD, MULTIPLY:
