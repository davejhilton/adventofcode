...
func part1(input []string) (string, error) {
	...
	log.Debugf("START: %d N, %d E, Facing: %s\n", state.NorthVal, state.EastVal, getDirectionName(state.Facing))
	...
	for _, dir := range directions {
		...
		log.Debugf("%s%d --> %d N, %d E, Facing: %s\n", string(dir.Action), dir.Value, state.NorthVal, state.EastVal, getDirectionName(state.Facing))
	}
...
func getDirectionName(d int) string {
	switch d {
	case NORTH:
		return "NORTH"
	case EAST:
...
func init() {
	challenges.RegisterChallengeFunc(2020, 12, 1, "day12.txt", part1)
	challenges.RegisterChallengeFunc(2020, 12, 2, "day12.txt", part2)
}
